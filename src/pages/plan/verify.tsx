...
export const getServerSideProps: GetServerSideProps = async (context) => {
  const { payment, message: urlMessage } = context.query;
  const message = typeof urlMessage === 'string' ? decodeURIComponent(urlMessage) : "";
  return {
    props: {
...
const PaymentVerificationPage: React.FC<PaymentVerificationPageProps> = ({ payment, message }) => {
  ...
  useEffect(() => {
    if (payment === "success") {
      setStatus("success");
      const paymentMessage = message || "Your payment was successful! Your plan has been activated.";
      verifySubscription();
      ...
      setStatus("error");
      const paymentMessage = message || "There was an issue processing your payment. Please try again.";
    } else {
      setStatus("loading");
    }
  }, [payment, message, router]);
  ...
  const verifySubscription = async () => {
    try {
      ...
      if (!response.data.has_subscription) {
        setStatus("error");
        // Directly use message prop here if needed
      }
    } catch (error) {
