...
const ProductCard: React.FC<ProductCardProps> = ({ product, isActive, onClick, onAddToCart, themeColor }) => {
  ...
  return (
    <motion.div
      className={`bg-white rounded-lg shadow-md overflow-hidden transition-shadow cursor-pointer h-full flex flex-col ${
        isActive ? "ring-2" : ""
      }`}
      style={{
    ...
    >
      {/* Product Image - adjust height for consistency */}
      <div className="relative h-36 sm:h-40 md:h-44 bg-gray-100">
        ...
        <div className="absolute top-2 left-2 flex flex-col space-y-1">
          ...
          {product.is_featured && (
            <span className="text-xs font-medium px-2 py-0.5 rounded bg-amber-500 text-white">FEATURED</span>
          )}

          {!isInStock && (
            <span className="text-xs font-medium px-2 py-0.5 rounded bg-gray-500 text-white">OUT OF STOCK</span>
          )}
        </div>
      </div>

      {/* Product Info - make text smaller on mobile */}
      <div className="p-2 sm:p-3 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-1 sm:mb-2">
          <h3 className="font-medium text-sm sm:text-base text-gray-900">{truncateText(product.name, 40)}</h3>
        </div>
        ...
        <div className="mt-auto flex items-center justify-between">
          <div className="flex items-center">
            ...
            {hasDiscount && (
              <span className="text-xs text-gray-500 line-through ml-2">₦{product.main_price || "0.00"}</span>
            )}
          </div>
          ...
          <button
            ...
            disabled={!isInStock}
            className={`p-1.5 sm:p-2 rounded-full ${!isInStock ? "bg-gray-200 cursor-not-allowed" : ""}`}
            style={{ backgroundColor: isInStock ? `${themeColor}15` : undefined }}
            aria-label="Add to cart"
          >
            <ShoppingCart size={16} style={{ color: isInStock ? themeColor : "gray" }} />
          </button>
        </div>
      </div>
    </motion.div>
